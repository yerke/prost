...
fn encode<T>(msg: &T) -> Result<Vec<u8>, prost::EncodeError>
where
    T: Message,
{
    let mut buf = Vec::new();
...
fn unknown_fields_are_kept() {
    let person_with_age = PersonWithAge {
        name: "Pwa".to_string(),
        age: 12,
    };
    let unrelated_message =
        unknown_fields::MessageWithAllTypes::decode(&encode(&person_with_age).unwrap()[..])
            .unwrap();
...
fn struct_without_unknown_field_set_discards_unknown_fields() {
    ...
    let reparsed_person_with_age = PersonWithAge::decode(&encode(&person).unwrap()[..]).unwrap();
...
fn unknown_fields_of_struct_with_nonstandard_set_name_are_kept() {
    ...
    let unrelated_message =
        NonstandardUnknownFieldSetName::decode(&encode(&person_with_age).unwrap()[..]).unwrap();
    let reparsed_person_with_age =
        PersonWithAge::decode(&encode(&unrelated_message).unwrap()[..]).unwrap();
...
fn overwrite_unknown_field_set_resets_unknown_fields() {
    ...
    let mut unrelated_message =
        EmptyMessage::decode(&encode(&person_with_age).unwrap()[..]).unwrap();
...
fn unknown_varint_fields_are_kept() {
    ...
    let unrelated_message = EmptyMessage::decode(&encode(&message).unwrap()[..]).unwrap();
    let reparsed_message =
        unknown_fields::MessageWithAllTypes::decode(&encode(&unrelated_message).unwrap()[..])
            .unwrap();
